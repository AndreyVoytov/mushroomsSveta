        ...
        if (shortInfo) {
            ...
            aims.forEach(aim => {
                ...
                if(wideImages.indexOf(aim.image) != -1){
                    labelShift += 5*(Math.floor(aim.count+10)/10);
                }

                let label = new Label(this.game, 550 + 90 + shiftX + labelShift, 3, "" + aim.count, { font: aims.length > 1? "bold 55px Arial" : "bold 60px Arial", fill: "#ffffff" });
                this.addChild(label);
