...
export default class StartLevelPanel extends ClosablePanel {
    ...
    constructor(game: Phaser.Game, screen: HouseScreen, x: number, y: number, aims: ForestAim[], level: number) {
        ...
        if (AdminService.isEditMode()) {
            ...
            let fastPlayLabel = new Label(this.game, 0, 0, LocalizationService.get(LocalizationKey.ui('fastPlay')), Label.COMMON_MEDIUM_STYLE);
    ...
    private getTaskTitle(currentTask: DiaryContentType): string {
        return LocalizationService.get(currentTask.titleForProgress || currentTask.title);
    }
