...
$sourceFontPath = Join-Path $PSScriptRoot "..\assets\base\fonts\bitmap\bm_balsamiq.fnt"
$sourceImagePath = Join-Path $PSScriptRoot "..\assets\base\fonts\bitmap\bm_balsamiq.png"
$destFontPath = Join-Path $PSScriptRoot "..\assets\base\fonts\bitmap\bm_hive_digits_legacy.fnt"
$destImagePath = Join-Path $PSScriptRoot "..\assets\base\fonts\bitmap\bm_hive_digits_legacy.png"

$outlineColor = [System.Drawing.ColorTranslator]::FromHtml("#6c2e11")
$fillColor = [System.Drawing.ColorTranslator]::FromHtml("#fde700")
...
function Get-OutlineDistanceSq([int]$thickness) {
    return [Math]::Max(2, ($thickness * $thickness) + 1)
}
...
foreach ($character in $characters) {
    ...
    if (-not $charNodes.ContainsKey($charId)) {
        throw "Character id $charId is missing in bm_balsamiq.fnt"
    }
...
[void]$fontText.AppendLine('  <info face="bm_hive_digits_legacy" size="72" bold="1" italic="0" charset="" unicode="1" stretchH="100" smooth="1" aa="1" padding="0,0,0,0" spacing="0,0" />')
...
[void]$fontText.AppendLine('    <page id="0" file="bm_hive_digits_legacy.png" />')
[void]$fontText.AppendLine('  </pages>')
